  ...
  <div class="row justify-content-left">
   <div class="col-md-6">
     <div class="card p-2  text-white"style="background-color:hsl(32, 63%, 16%);">
       <div class="card-header text-center text-white">{{ __('Add Area..') }}</div>
        <div class="card-body" style="background-color: hsl(26, 87%, 33%);">
             <form action="{{url('admin/add_area')}}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                 ...
                 <input type="number" class="form-control" id="charge" name="delivery_charge" value="{{old('delivery_charge')}}">             
                </div>
                <span class="text-danger text-weight-bold my-2"> {{$errors->first('delivery_charge')}}</span>
                </div>
              
            <button type="submit" class="btn btn-outline-warning w-100">Add Area</button>
            </form>            
        </div>
       </div>
     </div>

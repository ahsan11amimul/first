  ...
  <div class="row justify-content-left">
   <div class="col-md-6">
     <div class="card p-2 text-white" style="background-color:hsl(32, 63%, 16%);">
       <div class="card-header text-center text-white">{{ __('View category..') }}</div>
        <div class="card-body" style="background-color: hsl(26, 87%, 33%);">
             <form action="{{url('admin/edit_category/'.$category->id)}}" method="POST" enctype="multipart/form-data">
                ...
                <option value="">{{$category->products->count()}}</option> 
             ...
             <span class="text-danger text-weight-bold my-2"> {{$errors->first('url')}}</span> 
             ...
             <button type="submit" class="btn btn-outline-warning w-100">Edit Category</button>
            </form>           
        </div>
       </div>
     </div>
